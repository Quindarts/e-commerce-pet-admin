...
const ListProductPage = () => {
    ...
    const fetchProducts = async () => {
        try {
            const response = await fetch('https://e-commerce-pet-server-quindarts.vercel.app/products?offset=1&limit=6')
        ...
        <CustomListProduct>
            ...
            <Box className="contain mb-5 flex flex-wrap items-end">
                <Box className="contain__left flex">
                    <SearchBar className="h-[47px] p-[2px] lowercase" placeholder="Search product by" />
                </Box>

                <Box className="contain__right flex flex-wrap items-end justify-end gap-2">
                    <Dropdown label="Search type" className="w-[10rem]" size="sm" list={listSearch} />
